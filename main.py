...
def acquire_run_lock() -> bool:
    """Acquire an exclusive lock for a single pipeline run."""
    try:
...
def release_run_lock():
    """Release the exclusive run lock if this process owns it."""
    try:
        if LOCK_PATH.exists():
            LOCK_PATH.unlink()
...
def main():
    ...
    try:
        if not acquire_run_lock():
            logger.error("Another podcast summary run is already in progress. Exiting.")
            return
        lock_acquired = True
